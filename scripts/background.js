...
function removePlayerTabId(tabId) {
  ...
  if (index > -1) {
    ...
    if (index === 0 && LOCSTO.session.playersTabs.length > 0) {
      ...
      if (isPlaying === 'true') extensionOnMessageListener({ type: 'controlPlayer', command: 'play' });
    }
  }

  if (LOCSTO.session.playersData.hasOwnProperty(tabId)) delete LOCSTO.session.playersData[tabId];
          ...
          for (j = 0; j < aWindow.tabs.length; j++) {
            ...
            if (aTab.url.lastIndexOf('chrome', 0) !== 0) {
              chrome.tabs.executeScript(
                aTab.id,
                { file: '/scripts/hotkeys.js', runAt: 'document_start' },
                ignoreLastError
              );
            }
          }
